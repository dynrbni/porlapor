...
export default function Hero() {
  ...
  return (
    <section className="relative pt-32 pb-24 lg:pt-48 lg:pb-32 overflow-hidden bg-slate-50 flex items-center">
                ...
                <div className="w-full bg-slate-200 rounded-full h-2 overflow-hidden">
                  <div
                    className={`h-full transition-all duration-500 ${
                      searchResult.status.toUpperCase() === 'REJECTED'
                        ? 'bg-red-500'
                        : searchResult.status.toUpperCase() === 'RESOLVED'
                          ? 'bg-emerald-500'
                          : searchResult.status.toUpperCase() === 'IN_PROGRESS'
                            ? 'bg-blue-500'
                            : 'bg-amber-500'
                    }`}
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div
                    className={`flex-1 h-0.5 rounded-full ${
                      ['IN_PROGRESS', 'RESOLVED'].includes(searchResult.status.toUpperCase())
                        ? 'bg-blue-500'
                        : 'bg-slate-200'
                    }`}
                  />
                  <div className="flex-1 text-center text-xs">
                    ...
                    <span className="font-semibold text-slate-700">Proses</span>
                  </div>
                  <div
                    className={`flex-1 h-0.5 rounded-full ${
                      searchResult.status.toUpperCase() === 'RESOLVED' ? 'bg-emerald-500' : 'bg-slate-200'
                    }`}
                  />
                  <div className="flex-1 text-center text-xs">
                    <div
