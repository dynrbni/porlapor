...
import { Calendar, CheckCircle, Clock, Image as ImageIcon, Loader2, MapPin, ShieldAlert, X } from 'lucide-react';
...
export default function AdminReportDetailPanel({ reportId, onClose, onUpdated }: AdminReportDetailPanelProps) {
  ...
  const handleAddNote = async (event: React.FormEvent) => {
    ...
    setSavingNote(true);
    try {
      const res = await reportService.addOfficialNote(reportId, noteText.trim());
      setReport((prev) =>
        prev
          ? {
              ...prev,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-slate-950/70 px-4 py-8">
      <div className="w-full max-w-5xl bg-slate-50 rounded-3xl shadow-2xl border border-slate-200 overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-white">
          <div>
            <p className="text-xs font-semibold text-slate-400">Detail Laporan</p>
            <h2 className="text-lg font-bold text-slate-900">Panel Aksi Admin</h2>
          </div>
          <button
            onClick={onClose}
